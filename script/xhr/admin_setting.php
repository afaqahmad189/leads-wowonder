...
if ($f == 'admin_setting' and (Wo_IsAdmin() || Wo_IsModerator())) {
                    ...
                    if (!empty($group)) {
                        ...
                        if ($receipt->mode == 'create') {
                            ...
                            $notification_data_array = array(
                                ...
                                'url' => 'index.php?link1=private_group_created&id=' . $group->id,
                            ...
                            $notification_data_array = array(
                                'recipient_id' => $receipt->user_id,
                                'type' => 'admin_notification',
                                'url' => 'index.php?link1=private_group_joined&id=' . $group->id,
                                'text' => $wo['lang']['bank_pro'],
                                'type2' => 'no_name'
                            );
                            Wo_RegisterNotification($notification_data_array);
    ...
    if ($s == 'delete_job_category' && !empty($_POST['lang_key'])) {
        ...
        if (!empty($_GET['type']) && in_array($_GET['type'], $types)) {
            $lang_key = Wo_Secure($_POST['lang_key']);
            $category = $db->where('lang_key', $lang_key)->where('type', Wo_Secure($_GET['type']))->getOne(T_PRIVATE_JOB_CATEGORY);

            if (!empty($category)) {
                $db->where('lang_key', $lang_key)->delete(T_LANGS);
                $db->where('id', $category->id)->delete(T_PRIVATE_JOB_CATEGORY);

                // reset private group sub category
                if ($_GET['type'] == 'private-group') {
