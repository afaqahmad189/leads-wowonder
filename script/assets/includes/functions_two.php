...
function Wo_GetUserPaymentStatus($mode = 'create')
{
    ...
    $query      = mysqli_query($sqlConnect, "SELECT `id` FROM " . T_BANK_TRANSFER . " WHERE `type` = 'group' AND `mode` = '{$mode}' and `approved` = '0'");
    return Wo_Sql_Result($query, 0, 'id');
}
...
function Wo_PrivateGroupData($group_id = 0)
{
    ...
    $fetched_data['avatar']      = Wo_GetMedia($fetched_data['avatar']);
    $fetched_data['cover']       = Wo_GetMedia($fetched_data['cover']);
    $fetched_data['url']         = Wo_SeoLink('index.php?link1=private-timeline&u=' . $fetched_data['group_name']);
    $fetched_data['name']        = $fetched_data['group_title'];
    $fetched_data['category_id'] = $fetched_data['category'];
...
function Wo_GetPrivateGroupMember($group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $group_id  = Wo_Secure($group_id);
    $query     = " SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id} AND `user_id` = {$user_id} AND `active` = '1'";
    $sql_query = mysqli_query($sqlConnect, $query);
    return mysqli_fetch_assoc($sql_query);
}

function Wo_GetPrivateGroupMemberCategory($user_id)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
...
function Wo_IsPrivateNameExist($username, $active = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($username)) {
    ...
    $query   = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as usergroups FROM " . T_PRIVATE_GROUPS . " WHERE `group_name` = '{$username}' {$active_text}") or die(mysqli_error($sqlConnect));
    $fetched_data = mysqli_fetch_assoc($query);
    if ($fetched_data['usergroups'] > 0) {
        return array(
            true,
