...
function Wo_CreateSubadmin($user_id)
{
    global $sqlConnect;
    $query_one = mysqli_query($sqlConnect, "Select `user_id` from " . T_subadmin . " WHERE `user_id` = '{$user_id}'");
    if ($query_one) {
        if (mysqli_num_rows($query_one) > 0) {
            return ;
        }
        else {
            $query_two = mysqli_query($sqlConnect, "INSERT into " . T_subadmin . "(user_id,type) values({$user_id},'1')");
            if ($query_two) {
                return ;
            }
        }
...
function Wo_Deletesubadmin($user_id)
{
    global $sqlConnect,$query_one;
...
function Wo_UploadImage($file, $name, $type, $type_file, $user_id = 0, $placement = '')
{
    ...
    } else if ($placement == 'group' || $placement == 'private_group') {
        // private group id
    ...
    if ($type == 'cover') {
        ...
        } else if ($placement == 'private_group') {
            // update private group cover
            $query_one_delete_cover = mysqli_query($sqlConnect, " SELECT `cover` FROM " . T_PRIVATE_GROUPS . " WHERE `id` = " . $image_data['id'] . " AND `active` = '1'");
        } else if ($placement == 'event') {
        ...
        if (move_uploaded_file($file, $filename)) {
            ...
            if ($placement == 'page') {
                ...
                $update_data = Wo_UpdatePrivateGroupData($image_data['id'], $image_data);
            } else if ($placement == 'event') {
        ...
        } elseif ($placement == 'private_group') {
            $user_data = Wo_PrivateGroupData($image_data['id']);
        } else {
...
function Wo_RegisterNotification($data = array())
{
    ...
    // private group notifications - START
    $private_notifcation_query = '';
    $private_notifcation_query2 = '';
    if (!empty($data['private_id']) && $data['private_id'] > 0) {
        $group = Wo_PrivateGroupData($data['private_id']);
        if (!isset($group['id'])) {
        }
        $group_id = Wo_Secure($group['id']);
        $private_notifcation_query = '`private_id`,';
        $private_notifcation_query2 = "{$group_id}, ";
    }
    // END
...
function Wo_PostData($post_id, $placement = '', $limited = '', $comments_limit = 0)
{
    ...
    $story['private_recipient_exists'] = false;
    ...
    if ($placement != 'admin') {
        ...
        if (!empty($story['private_id'])) {
            if ($wo['config']['private_groups'] == 0) {
                return false;
            }
            $story['private_recipient_exists'] = true;
            $story['private_recipient'] = Wo_PrivateGroupData($story['private_id']);
            if ($wo['loggedin'] == true) {
                if ($story['publisher']['user_id'] != $wo['user']['user_id']) {
                    if (Wo_IsPrivateGroupOnwer($story['private_id']) === false) {
                        if (Wo_IsPrivateGroupJoined($story['private_id']) === false && (!Wo_IsAdmin() || Wo_IsModerator())) {
                            return false;
                        }
                    }
                }
        ...
        if ($story['postPrivacy'] == 2) {
            if ($wo['loggedin'] == true) {
                if (!empty($story['publisher']['page_id'])) {
                    ...
                    if ($story['publisher']['user_id'] != $wo['user']['user_id']) {
                        if (Wo_IsFollowing($story['publisher']['user_id'], $wo['user']['user_id']) === false && empty($story['group_id']) && empty($story['private_id'])) {
                            return false;
                        }
                    }
                }
    ...
    if ($wo['loggedin'] == true) {
        ...
        if (Wo_IsBlocked($story['user_id']) || Wo_IsBlocked($story['recipient_id'])) {
            if (empty($story['group_id']) || empty($story['private_id'])) {
                return false;
            }
        }
    }
...
function Wo_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'private_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0))
{
    global $wo, $sqlConnect;
    ...
    if (!empty($data['private_id']) && is_numeric($data['private_id']) && $data['private_id'] > 0) {
        $data['private_id'] = Wo_Secure($data['private_id']);
        $Wo_private_publisher = Wo_PrivateGroupData($data['private_id']);
    }
    ...
    if (isset($Wo_publisher['user_id'])) {
    ...
    } else if (isset($Wo_private_publisher['id'])) {
        $private_id = Wo_Secure($Wo_private_publisher['id']);
        $query_text .= " AND (`private_id` = {$private_id}) AND `id` NOT IN (SELECT `post_id` from " . T_PINNED_POSTS . " WHERE `private_id` = {$private_id})";
        switch ($data['filter_by']) {
            case 'text':
                $query_text .= " AND `postText` <> '' AND `postFile` = '' AND `postYoutube` = '' AND `postFacebook` = ''  AND `postVimeo` = ''  AND `postDailymotion` = '' AND `postSoundCloud` = '' ";
                break;
        ...
        $private_not_joined = array();
        $query_private = "SELECT `private_id` FROM " . T_POSTS . " WHERE (`user_id` IN (SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$logged_user_id} AND `active` = '1') AND `private_id` <> 0 AND `private_id` NOT IN (SELECT `private_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `user_id` = '{$logged_user_id}' AND `active` = '1'))";
        $query_private = mysqli_query($sqlConnect, $query_private);
        while ($fetched_data_private = mysqli_fetch_assoc($query_private)) {
            if (!in_array($fetched_data_private['private_id'], $private_not_joined)) {
                $private_not_joined[] = $fetched_data_private['private_id'];
            }
        }
        ...
        if ($add_filter_query == true) {
            ...
            AND (
                  ...
                  OR `private_id` IN (SELECT `id` FROM " . T_PRIVATE_GROUPS . " WHERE `user_id` = {$logged_user_id})
                  ...
                  OR `private_id` IN (SELECT `group_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `user_id` = {$logged_user_id} AND `user_id` IN (SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$logged_user_id} AND `active` = '1')
                  )
            )";
        }
        ...
        if (!empty($private_not_joined)) {
            $implode_private = implode($private_not_joined, ',');
            $query_text .= " AND `private_id` NOT IN ($implode_private)";
        }
        switch ($data['filter_by']) {
            case 'text':
                $query_text .= " AND `postText` <> '' AND `postFile` = '' AND `postYoutube` = '' AND `postFacebook` = ''  AND `postVimeo` = ''  AND `postDailymotion` = '' AND `postSoundCloud` = '' ";
...
function Wo_GetPinnedPost($user_id, $type = '')
{
    ...
    if ($type == 'page') {
        ...
        // get pinned posts for private groups
        $query_type = 'private_id';
    }
    $data = array();
...
function Wo_PinPost($post_id = 0, $type = '', $id = 0)
{
    ...
    if ($type == 'page') {
        ...
        // for private groups
        if (Wo_IsPrivateGroupOnwer($id) === false) {
            return false;
        }
        $where_delete_query = " WHERE `private_id` = {$id} AND `active` = '1'";
        $where_insert_query = " (`private_id`, `post_id`, `active`) VALUES ({$id}, {$post_id}, '1')";
    } else if ($type == 'event') {
...
//abdul custom start
function Wo_ProductCategoryFromDB($lang = 'english')
{
    global $sqlConnect, $wo;
    $data = array();
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRODUCT_CATEGORY);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $data[] = $fetched_data;
    }
    return $data;
...
function Wo_ProductCategoryNameFromDB($lang = 'english')
{
    global $sqlConnect, $wo;
    $data = array();
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRODUCT_CATEGORY);
...
function Wo_ProductCategoryFromByid($id)
{
    global $sqlConnect, $wo;
    $data = array();
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRODUCT_CATEGORY . " WHERE `id` = '{$id}'");
//    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRODUCT_CATEGORY . " where id=".$id);
    return mysqli_fetch_assoc($query);
}
