...
function Wo_CountAllData($type)
{
    ...
    if ($type == 'user') {
        ...
        // add dependency for private groups
        $type_table = T_PRIVATE_GROUPS;
        $type_id    = 'id';
    }
    $type_id      = Wo_Secure($type_id);
...
function Wo_IsNameExist($username, $active = 0)
{
    ...
    // check if name exists of a private group
    $query   = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as privategroups FROM " . T_PRIVATE_GROUPS . " WHERE `group_name` = '{$username}'") or die(mysqli_error($sqlConnect));
    $fetched_data = mysqli_fetch_assoc($query);
    if ($fetched_data['privategroups'] > 0) {
        return array(
            true,
            'type' => 'private_group'
        );
...
function Wo_CountGroupData($type)
{
    ...
    if ($type == 'members') {
        ...
        $where      = "`active` = '0'";
        $type_id    = 'id';
    } else if ($type == 'private-members') {
        // add dependency for private group members
        $type_table = T_PRIVATE_MEMBERS;
        $where      = "`active` = '1'";
        $type_id    = 'id';
    } else if ($type == 'private-posts') {
        // add dependency for private group posts
        $type_table = T_POSTS;
        $where      = "`private_id` <> 0";
        $type_id    = 'id';
    } else if ($type == 'private-requests') {
        // add dependency for private group member requests
        $type_table = T_PRIVATE_MEMBERS;
        $where      = "`active` = '0'";
        $type_id    = 'id';
    }
...
// private group functions - START
function Wo_IsCanPrivateGroupUpdate($group_id, $page)
{
    global $sqlConnect, $wo;
    $array = array('general', 'avatar', 'members', 'analytics', 'delete_group');
    if ($wo['loggedin'] == false) {
        return false;
    }
    if (empty($group_id) || !is_numeric($group_id) || $group_id < 0 || empty($page) || !in_array($page, $array)) {
    ...
    $page = Wo_Secure($page);
    $group_id = Wo_Secure($group_id);
    $query = mysqli_query($sqlConnect, " SELECT COUNT(*) FROM " . T_PRIVATE_ADMINS . " WHERE `group_id` = {$group_id} AND `user_id` = {$user_id} AND `{$page}` = '1'");
    return (Wo_Sql_Result($query, '0') == 1) ? true : false;
}

function Wo_GetAllowedPrivateGroupPages($group_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $array = array('general' => 'general-setting', 'avatar' => 'avatar-setting', 'members' => 'group-members', 'analytics' => 'analytics', 'delete_group' => 'delete-group');
    $data = array();
    $user_id = $wo['user']['id'];
    $group_id = Wo_Secure($group_id);
    $query = mysqli_query($sqlConnect, " SELECT * FROM " . T_PRIVATE_ADMINS . " WHERE `group_id` = {$group_id} AND `user_id` = {$user_id}");
    $fetched_data = mysqli_fetch_assoc($query);
    if (!empty($fetched_data)) {
        foreach ($fetched_data as $key => $value) {
            if (in_array($key, array_keys($array)) && $value == 1) {
...
function Wo_RegisterPrivateGroup($registration_data = array())
{
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    ...
    if (!empty($registration_data['category'])) {
        if (!in_array($registration_data['category'], array_keys($wo['private_categories']))) {
            $registration_data['category'] = 1;
        }
    }
    $registration_data['registered'] = date('n') . '/' . date("Y");
    $fields                          = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data                            = '\'' . implode('\', \'', $registration_data) . '\'';
    $query                           = mysqli_query($sqlConnect, "INSERT INTO " . T_PRIVATE_GROUPS . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    } else {
        return false;
...
function Wo_GetMyPrivateGroups()
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $data       = array();
    $user_id    = Wo_Secure($wo['user']['user_id']);
    $query_text = "SELECT `id` FROM " . T_PRIVATE_GROUPS . " WHERE `user_id` = {$user_id} 
                   OR `id` IN (SELECT `group_id` FROM " . T_PRIVATE_ADMINS . " WHERE `user_id` = {$user_id})";
    $query_one  = mysqli_query($sqlConnect, $query_text);
    if (mysqli_num_rows($query_one) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            if (is_array($fetched_data)) {
                $data['type'] = 'self';
                $data['group'] = Wo_PrivateGroupData($fetched_data['id']);
            }
        }
    } else {
        $query     = " SELECT `group_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `user_id` = {$user_id} AND `active` = '1' ORDER BY `id`";
        $sql_query = mysqli_query($sqlConnect, $query);
        if (mysqli_num_rows($sql_query) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
                $data['type'] = 'join';
                $data['group'] = Wo_PrivateGroupData($fetched_data['group_id']);
            }
        }
    }
...
function Wo_IsPrivateGroupOnwer($group_id = 0, $user_id = 0)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query = mysqli_query($sqlConnect, " SELECT COUNT(`user_id`) FROM " . T_PRIVATE_GROUPS . " WHERE `id` = {$group_id} AND `user_id` = {$user_id}");
    return (Wo_Sql_Result($query, '0') == 1 || Wo_IsPrivateGroupUserExists($user_id, $group_id)) ? true : false;
}

function Wo_GetPrivateGroupOnwer($group_id = 0)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query = mysqli_query($sqlConnect, " SELECT A.`user_id`, B.`category` FROM " . T_PRIVATE_GROUPS . " AS A JOIN " . T_PRIVATE_MEMBERS . " AS B ON A.`user_id` = B.`user_id` WHERE A.`id` = {$group_id}");
    return mysqli_fetch_assoc($query);
}

function Wo_PrivateGroupExists($group_name = '')
{
    global $sqlConnect;
    if (empty($group_name)) {
        return false;
    ...
    $group_name = Wo_Secure($group_name);
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_GROUPS . " WHERE `group_name`= '{$group_name}'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_GetUserPaymentStatus($mode = 'create')
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query      = mysqli_query($sqlConnect, "SELECT `id` FROM " . T_BANK_TRANSFER . " WHERE user_id = '{$user_id}' AND `type` = 'group' AND `mode` = '{$mode}' and `approved` = '0'");
    return Wo_Sql_Result($query, 0, 'id');
}

function Wo_PrivateGroupIdFromGroupname($group_name = '')
{
    global $sqlConnect;
    if (empty($group_name)) {
        return false;
    ...
    $group_name = Wo_Secure($group_name);
    $query      = mysqli_query($sqlConnect, "SELECT `id` FROM " . T_PRIVATE_GROUPS . " WHERE `group_name` = '{$group_name}'");
    return Wo_Sql_Result($query, 0, 'id');
}

function Wo_PrivateGroupData($group_id = 0)
{
    global $wo, $sqlConnect, $cache;
    if (empty($group_id) || !is_numeric($group_id) || $group_id < 1) {
        return false;
    ...
    $data            = array();
    $group_id        = Wo_Secure($group_id);
    $query_one       = "SELECT * FROM " . T_PRIVATE_GROUPS . " WHERE `id` = {$group_id}";
    $hashed_group_Id = md5($group_id);
    if ($wo['config']['cacheSystem'] == 1) {
        $fetched_data = $cache->read($hashed_group_Id . '_PRIVATE_GROUP_Data.tmp');
        if (empty($fetched_data)) {
            $sql          = mysqli_query($sqlConnect, $query_one);
            $fetched_data = mysqli_fetch_assoc($sql);
            $cache->write($hashed_group_Id . '_PRIVATE_GROUP_Data.tmp', $fetched_data);
        }
    } else {
        $sql          = mysqli_query($sqlConnect, $query_one);
        $fetched_data = mysqli_fetch_assoc($sql);
    ...
    $fetched_data['name']        = $fetched_data['group_title'];
    $fetched_data['category_id'] = $fetched_data['category'];
    $fetched_data['type']        = 'private-group';
    $fetched_data['username']    = $fetched_data['group_name'];
...
function Wo_IsPrivateGroupJoined($group_id = 0, $user_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $group_id  = Wo_Secure($group_id);
    $user_id = Wo_Secure($user_id);
    $query_one = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_MEMBERS . " WHERE `user_id` = '{$user_id}' AND `group_id` = {$group_id}");
    return (Wo_Sql_Result($query_one, 0) == 1) ? true : false;
}

function Wo_GetUserIdFromPrivateGroupId($group_id = 0)
{
    global $sqlConnect;
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
        return false;
    ...
    $group_id      = Wo_Secure($group_id);
    $query_one     = "SELECT `user_id` FROM " . T_PRIVATE_GROUPS . " WHERE `id` = {$group_id}";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql_query_one) == 1) {
        $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
        return $sql_fetch_one['user_id'];
...
function Wo_DeletePrivateGroup($group_id = 0)
{
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $group_id = Wo_Secure($group_id);
    if (Wo_IsAdmin() === false && Wo_IsModerator() === false) {
        if (Wo_IsPrivateGroupOnwer($group_id) === false) {
            return false;
        }
    }
    $query_one_delete_photos = mysqli_query($sqlConnect, " SELECT `avatar`,`cover` FROM " . T_PRIVATE_GROUPS . " WHERE `id` = {$group_id}");
    $fetched_data            = mysqli_fetch_assoc($query_one_delete_photos);
    if (isset($fetched_data['avatar']) && !empty($fetched_data['avatar']) && $fetched_data['avatar'] != $wo['groupDefaultAvatar']) {
        @unlink($fetched_data['avatar']);
    }
    ...
    $query_two_delete_media = mysqli_query($sqlConnect, " SELECT `postFile` FROM " . T_POSTS . " WHERE `private_id` = {$group_id}");
    if (mysqli_num_rows($query_two_delete_media) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query_two_delete_media)) {
            if (isset($fetched_data['postFile']) && !empty($fetched_data['postFile'])) {
                @unlink($fetched_data['postFile']);
    ...
    $query_four_delete_media = mysqli_query($sqlConnect, "SELECT `id`,`post_id` FROM " . T_POSTS . " WHERE `private_id` = {$group_id}");
    if (mysqli_num_rows($query_four_delete_media) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query_four_delete_media)) {
            $delete_posts = Wo_DeletePost($fetched_data['id']);
            $delete_posts = Wo_DeletePost($fetched_data['post_id']);
    ...
    if ($wo['config']['cacheSystem'] == 1) {
        $cache->delete(md5($group_id) . '_PRIVATE_GROUP_Data.tmp');
        $query_two = mysqli_query($sqlConnect, "SELECT `id`,`post_id` FROM " . T_POSTS . " WHERE `private_id` = {$group_id}");
        if (mysqli_num_rows($query_two) > 0) {
            while ($fetched_data_two = mysqli_fetch_assoc($query_two)) {
                $cache->delete(md5($fetched_data_two['id']) . '_PRIVATE_GROUP_Data.tmp');
                $cache->delete(md5($fetched_data_two['post_id']) . '_PRIVATE_GROUP_Data.tmp');
            }
        }
    }
    $query_one = mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_GROUPS . " WHERE `id` = {$group_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_ALLOWED_CATEGORY . " WHERE `group_id` = {$group_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_INVITES . " WHERE `group_id` = {$group_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_NOTIFICATION . " WHERE `private_id` = {$group_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_ADMINS . " WHERE `group_id` = {$group_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_SUGGESTIONS . " WHERE `group_id` = {$group_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_POSTS . " WHERE `private_id` = {$group_id}");
    if ($query_one) {
        return true;
    }
}

function Wo_CountPrivateGroupMembers($group_id = 0)
{
    global $wo, $sqlConnect;
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
        return false;
    ...
    $group_id     = Wo_Secure($group_id);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`group_id`) AS count FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id} AND `active` = '1' ");
    $fetched_data = mysqli_fetch_assoc($query);
    return $fetched_data['count'];
}

function Wo_PrivateGroupExistsByID($id)
{
    global $sqlConnect;
    if (empty($id)) {
        return false;
    ...
    $id    = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_GROUPS . " WHERE `id`= '{$id}' AND `active` = '1'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_GetPrivateCategories($all = true)
{
    global $sqlConnect, $wo;
    $data = array();

    if ($all) {
        $sql = "SELECT * FROM " . T_PRIVATE_CATEGORY;
    } else {
        $sql = "SELECT * FROM " . T_PRIVATE_CATEGORY . " WHERE id IN (SELECT category_id from " . T_PRIVATE_JOB_CATEGORY . ")";
    }
...
function Wo_GetPrivateJobCategories()
{
    global $sqlConnect, $wo;
    $wo['private_job_categories'] = array();
    $categories = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRIVATE_JOB_CATEGORY);
    if ($categories) {
        while ($fetched_data = mysqli_fetch_assoc($categories)) {
            $fetched_data['lang'] = $wo['lang'][$fetched_data['lang_key']];
            $wo['private_job_categories'][$fetched_data['category_id']][] = $fetched_data;
        }
        return true;
    }
    return false;
...
function Wo_GetPrivateJoinButton($group_id = 0)
{
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (Wo_IsPrivateGroupOnwer($group_id)) {
        return false;
    }
    $group_id = Wo_Secure($group_id);
    $group    = $wo['join'] = Wo_PrivateGroupData($group_id);
    if (!isset($wo['join']['id'])) {
        return false;
    }
    $logged_user_id        = Wo_Secure($wo['user']['user_id']);
    $leave_button          = 'buttons/private-leave';
    if (Wo_IsPrivateGroupJoined($group_id, $logged_user_id) === true) {
        return Wo_LoadPage($leave_button);
    } else {
        return false;
    }
...
function Wo_GetPrivateGroupMembers($group_id = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
    ...
    $group_id  = Wo_Secure($group_id);
    $query     = " SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id} AND `active` = '1'";
    $sql_query = mysqli_query($sqlConnect, $query);
    while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
        $data[] = Wo_UserData($fetched_data['user_id']);
    }
...
function Wo_GetPrivateGroupMember($group_id = 0, $user_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $group_id  = Wo_Secure($group_id);
    $query     = " SELECT * FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id} AND `user_id` = {$user_id} AND `active` = '1'";
    $sql_query = mysqli_query($sqlConnect, $query);
    return mysqli_fetch_assoc($sql_query);
}

function Wo_GetPrivateGroupMemberCategory($user_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id      = Wo_Secure($user_id);
    $query = "SELECT B.`id`, B.`lang_key` FROM " . T_PRIVATE_MEMBERS . " as A JOIN " . T_PRIVATE_JOB_CATEGORY . " AS B ON A.`category` = B.`id` WHERE A.`user_id` = '{$user_id}' AND A.active = '1'";
    $sql_query = mysqli_query($sqlConnect, $query);
    if (mysqli_num_rows($sql_query) > 0) {
        $row = mysqli_fetch_assoc($sql_query);

        return $wo['lang'][$row['lang_key']];
    }

    return '-';
}

function Wo_CountPrivateGroupsNotMember($group_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $group_id     = Wo_Secure($group_id);
    $query_one    = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$user_id} AND `active` = '1' AND `following_id` NOT IN (SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id})");
    $fetched_data = mysqli_fetch_assoc($query_one);
    return $fetched_data['count'];
}
function Wo_GetPrivateGroupsNotMember($group_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $group_id  = Wo_Secure($group_id);
    $user_id   = Wo_Secure($wo['user']['user_id']);
    $query_one = mysqli_query($sqlConnect, "SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$user_id} AND `active` = '1' AND `following_id` NOT IN (SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id})");
    while ($fetched_data = mysqli_fetch_assoc($query_one)) {
        $data[] = Wo_UserData($fetched_data['following_id']);
    }
    return $data;
...
function Wo_CanBeOnPrivateGroup($group_id)
{
    global $sqlConnect;
    if (empty($group_id)) {
        return false;
    ...
    $group = Wo_PrivateGroupData($group_id);
    if (empty($group)) {
        return false;
    }

    if (Wo_IsPrivateGroupJoined($group_id) === true) {
        return true;
    } else {
        return false;
    }
...
function Wo_GetPrivateGroupPostPublisherBox($group_id = 0)
{
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $group_id = Wo_Secure($group_id);
    $continue = false;
    if (Wo_CanBeOnPrivateGroup($group_id) === true) {
        $group = Wo_PrivateGroupData($group_id);
...
function Wo_CountPrivateGroupPosts($group_id = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
    ...
    $group_id     = Wo_Secure($group_id);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`id`) AS count FROM " . T_POSTS . " WHERE `private_id` = {$group_id}");
    $fetched_data = mysqli_fetch_assoc($query);
    return $fetched_data['count'];
}

function Wo_CountPrivateJoinedThisWeek($group_id = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    $time = strtotime("-1 week");
    ...
    $group_id     = Wo_Secure($group_id);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`group_id`) AS count FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id} AND `active` = '1' AND (`time` between {$time} AND " . time() . ")");
    $fetched_data = mysqli_fetch_assoc($query);
    return $fetched_data['count'];
}

function Wo_UpdatePrivateGroupData($group_id = 0, $update_data)
{
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $impload   = implode(', ', $update);
    $query_one = " UPDATE " . T_PRIVATE_GROUPS . " SET {$impload} WHERE `id` = {$group_id} ";
    $query     = mysqli_query($sqlConnect, $query_one);
    if ($wo['config']['cacheSystem'] == 1) {
        $cache->delete(md5($group_id) . '_PRIVATE_GROUP_Data.tmp');
    }
...
function Wo_PrivateGroupActive($group_name)
{
    global $sqlConnect;
    if (empty($group_name)) {
        return false;
    ...
    $group_name = Wo_Secure($group_name);
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_GROUPS . "  WHERE `group_name` = '{$group_name}' AND `active` = '1'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_GetPrivateGroupAdminInfo($user_id, $group_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || empty($group_id) || !is_numeric($group_id) || empty($user_id) || !is_numeric($user_id)) {
        return false;
    ...
    $group_id = Wo_Secure($group_id);
    $user_id = Wo_Secure($user_id);
    $sql     = " SELECT * FROM " . T_PRIVATE_ADMINS . " WHERE `group_id` = {$group_id} AND `user_id` = {$user_id}";
    $data    = array();
    $query   = mysqli_query($sqlConnect, $sql);
    return mysqli_fetch_assoc($query);
}

function Wo_GetPrivateGroupSettingMembers($group_id = 0, $limit = 0, $offset = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
    ...
    $group_id  = Wo_Secure($group_id);
    $query     = " SELECT `user_id`,`id` FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id} AND `active` = '1' $offset_query $limit_query";
    $sql_query = mysqli_query($sqlConnect, $query);
    while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
        $user_data = Wo_UserData($fetched_data['user_id']);
        $user_data['member_id'] = $fetched_data['id'];
...
function Wo_UpdatePrivateGroupAdminData($group_id, $update_data, $user_id)
{
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $impload   = implode(', ', $update);
    $query_one = " UPDATE " . T_PRIVATE_ADMINS . " SET {$impload} WHERE `group_id` = {$group_id} AND `user_id` = '{$user_id}' ";
    $query     = mysqli_query($sqlConnect, $query_one);
    if ($query) {
        return true;
    } else {
...
function Wo_CanSendInvite($user_id = 0, $group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (Wo_IsPrivateGroupJoined($group_id, $logged_user_id) === false) {
        return false;
    }

    if (Wo_IsPrivateGroupJoined($group_id, $user_id) === true) {
        return false;
    }
...
function Wo_RegisterPrivateGroupInvite($user_id = 0, $group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id  = Wo_Secure($user_id);
    $group_id = Wo_Secure($group_id);
    if (Wo_CanSendInvite($user_id, $group_id) == false) {
        return false;
    }

    $logged_user_id = Wo_Secure($wo['user']['user_id']);
    $token = Wo_GenerateRandomString(12);
    ...
    $invite_id = Wo_RegisterInvite($registration_data);
    if (!empty($invite_id)) {
        return $token;
    } else {
        return false;
...
function Wo_RegisterInvite($registration_data = array())
{
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    ...
    $fields = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data   = '\'' . implode('\', \'', $registration_data) . '\'';
    $query  = mysqli_query($sqlConnect, "INSERT INTO " . T_PRIVATE_INVITES . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    } else {
        return false;
...
function Wo_GenerateRandomString($length = 10)
{
...
function Wo_isUserInvited($user_id = 0, $group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id  = Wo_Secure($user_id);
    $group_id = Wo_Secure($group_id);
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_INVITES . " WHERE `recipient_id` = '{$user_id}' && `group_id` = '{$group_id} ORDER BY `time` DESC LIMIT 1'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_PrivateGroupInvitationExists($token = '', $group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_INVITES . " WHERE `token` = '{$token}' && `recipient_id` = '{$user_id}' && `group_id` = '{$group_id}' && `active` = '0'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_GetPrivateGroupInvitation($token = '', $group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query      = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRIVATE_INVITES . " WHERE `token` = '{$token}' && `recipient_id` = '{$user_id}' && `group_id` = '{$group_id}' && `active` = '0'");
    return mysqli_fetch_assoc($query);
}

function Wo_GetAllowedCategories($group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $group_id = Wo_Secure($group_id);
    $categories = mysqli_query($sqlConnect, "SELECT B.`id`, B.`lang_key` FROM " . T_PRIVATE_ALLOWED_CATEGORY . " as A JOIN " . T_PRIVATE_JOB_CATEGORY . " AS B ON A.`jobcategory_id` = B.`id` WHERE A.`group_id` = '{$group_id}' AND A.active = '0'");
    if ($categories) {
        while ($fetched_data = mysqli_fetch_assoc($categories)) {
            $data[$fetched_data['id']] = $wo['lang'][$fetched_data['lang_key']];
        }
...
function Wo_PrivateCanInvite($group_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query_one    = mysqli_query($sqlConnect, "SELECT COUNT(`group_id`) AS count FROM " . T_PRIVATE_MEMBERS . " WHERE `group_id` = {$group_id} ");
    $members      = mysqli_fetch_assoc($query_one);

    $query_two    = mysqli_query($sqlConnect, "SELECT COUNT(`group_id`) AS count FROM " . T_PRIVATE_ALLOWED_CATEGORY . " WHERE `group_id` = {$group_id} ");
    $allowed      = mysqli_fetch_assoc($query_two);

    return ($members['count'] > $allowed['count']) ? false : true;
}

function Wo_LeavePrivateGroup($group_id = 0, $user_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $group_id = Wo_Secure($group_id);
    $user_id  = Wo_Secure($user_id);
    if (Wo_IsPrivateGroupJoined($group_id, $user_id) === false) {
        return false;
    }

    $member = Wo_GetPrivateGroupMember($group_id, $user_id);
    $job_category = $member['category'];
    $query = mysqli_query($sqlConnect, " DELETE FROM " . T_PRIVATE_MEMBERS . " WHERE `user_id` = {$user_id} AND `group_id` = '{$group_id}'");
    if ($query) {
        ...
        @mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_ADMINS . " WHERE `user_id` = {$user_id} AND `group_id` = {$group_id}");
        @mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_INVITES . " WHERE `recipient_id` = {$user_id} AND `group_id` = {$group_id}");
        return true;
    }
}

function Wo_CountPrivateGroupsNotCategories($group_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    // get group main category
    $group_data = Wo_PrivateGroupData($group_id);
    $category_id = $group_data['category_id'];

    // get group owner's job category
    $group_owner = Wo_GetPrivateGroupOnwer($group_id);
    $categories = array($group_owner['category']);

    $query_one    = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRIVATE_ALLOWED_CATEGORY . " WHERE `group_id` = {$group_id}");
    while ($fetched_data = mysqli_fetch_assoc($query_one)) {
        $categories[] = $fetched_data['jobcategory_id'];
    }

    $fields = '"' . implode('", "', $categories) . '"';
    $query_two    = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_PRIVATE_JOB_CATEGORY . " WHERE `category_id` = {$category_id} AND `id` NOT IN ({$fields})");
    $fetched_data = mysqli_fetch_assoc($query_two);
    return $fetched_data['count'];
}
function Wo_GetPrivateGroupsNotCategories($group_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $fields = '"' . implode('", "', $categories) . '"';
    $query_two    = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRIVATE_JOB_CATEGORY . " WHERE `category_id` = {$category_id} AND `id` NOT IN ({$fields})");
    if ($query_two) {
        while ($fetched_data = mysqli_fetch_assoc($query_two)) {
            $fetched_data['lang'] = $wo['lang'][$fetched_data['lang_key']];
            $data[$fetched_data['category_id']][] = $fetched_data;
        }
    }
...
function Wo_SearchForMembers($search_qeury, $type)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if ($type == 'referral' || $type == 'forward') {
        $sql = " SELECT A.`group_id`, A.`user_id` FROM " . T_PRIVATE_MEMBERS . " AS A JOIN " . T_USERS . " AS B ON A.user_id = B.user_id WHERE ((`username` LIKE '%$search_qeury%') OR (`first_name` LIKE '%$search_qeury%') OR (`last_name` LIKE '%$search_qeury%') OR (CONCAT(B.`first_name`, ' ', B.`last_name`) LIKE '%$search_qeury%')) AND A.`active` = '1' AND A.`user_id` != '{$user_id}' AND B.`active` = '1' GROUP BY `user_id` ORDER By `first_name` LIMIT 10 ";
    } else {
        $sql = " SELECT B.group_id, B.user_id FROM " . T_PRIVATE_REFERRALS . " AS A JOIN " . T_PRIVATE_MEMBERS . " AS B ON A.`sender_id` = B.`user_id` JOIN " . T_USERS . " AS C ON A.`sender_id` = C.`user_id`  WHERE ((C.`username` LIKE '%$search_qeury%') OR (C.`first_name` LIKE '%$search_qeury%') OR (C.`last_name` LIKE '%$search_qeury%') OR (CONCAT(C.`first_name`, ' ', C.`last_name`) LIKE '%$search_qeury%')) AND A.`status` = '1' AND A.`sender_id` != '{$user_id}' AND B.`active` = '1' AND C.`active` = '1' GROUP BY `user_id` ORDER By `first_name` LIMIT 10 ";
    }

    $query = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        if ($type == 'thanks') {
            if (Wo_CanSearchUser($fetched_data['user_id']) != true) {
                continue;
            }
        }

        $group = Wo_PrivateGroupData($fetched_data['group_id']);
        $new_data = Wo_userData($fetched_data['user_id']);
        $new_data['group_name'] = $group['group_title'];
...
function Wo_RegisterReferral($registration_data = array())
{
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    ...
    $fields                          = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data                            = '\'' . implode('\', \'', $registration_data) . '\'';
    $query                           = mysqli_query($sqlConnect, "INSERT INTO " . T_PRIVATE_REFERRALS . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    } else {
        return false;
...
function Wo_CanSeeReferral()
{
    ...
    $query_two    = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_PRIVATE_REFERRALS . " WHERE `recipient_id` = {$user_id} AND `status` = '0'");
    $fetched_data = mysqli_fetch_assoc($query_two);
    return ($fetched_data['count'] > 0) ? true : false;
}

function Wo_CanSendThanks()
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query_two    = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_PRIVATE_REFERRALS . " WHERE `recipient_id` = {$user_id} AND `status` = '1'");
    $fetched_data = mysqli_fetch_assoc($query_two);
    return ($fetched_data['count'] > 0) ? true : false;
}

function Wo_CanSearchUser($user_id = 0)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $logged_user_id = Wo_Secure($wo['user']['user_id']);
    $query_two    = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_PRIVATE_THANKS . " WHERE `sender_id` = {$logged_user_id} AND `recipient_id` = {$user_id}");
    $fetched_data = mysqli_fetch_assoc($query_two);
    return ($fetched_data['count'] > 0) ? false : true;
}

function Wo_RegisterThanks($registration_data = array())
{
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    ...
    $fields                          = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data                            = '\'' . implode('\', \'', $registration_data) . '\'';
    $query                           = mysqli_query($sqlConnect, "INSERT INTO " . T_PRIVATE_THANKS . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    } else {
        return false;
...
function Wo_RegisterSuggestions($registration_data = array())
{
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    ...
    $fields                          = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data                            = '\'' . implode('\', \'', $registration_data) . '\'';
    $query                           = mysqli_query($sqlConnect, "INSERT INTO " . T_PRIVATE_SUGGESTIONS . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    } else {
        return false;
...
function Wo_GetAllSuggestions()
{
    global $wo, $sqlConnect;

    $suggestions = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRIVATE_SUGGESTIONS);
    if ($suggestions) {
        $data = array();
        while ($fetched_data = mysqli_fetch_assoc($suggestions)) {
            $data[] = $fetched_data;
        }
...
function Wo_GetUserPrivateGroup($user_id = 0)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id = Wo_Secure($wo['user']['user_id']);
    $sql_query = mysqli_query($sqlConnect, " SELECT `group_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `user_id` = {$user_id} AND `active` = '1'");

    if (mysqli_num_rows($sql_query) > 0) {
        $fetched_data = mysqli_fetch_assoc($sql_query);

        return Wo_PrivateGroupData($fetched_data['group_id']);
    }

    return false;
}

function Wo_GetReferral($referral_id = 0)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id = Wo_Secure($wo['user']['user_id']);
    if (!isset($referral_id) or empty($referral_id) or !is_numeric($referral_id) or $referral_id < 1) {
        $sql = "SELECT * FROM " . T_PRIVATE_REFERRALS . " WHERE `recipient_id` = {$user_id} AND `status` = '0' ORDER BY id LIMIT 1";
    } else {
        $sql = "SELECT * FROM " . T_PRIVATE_REFERRALS . " WHERE `id` = {$referral_id}";
    }

    $query_two    = mysqli_query($sqlConnect, $sql);
    $fetched_data = mysqli_fetch_assoc($query_two);
    return $fetched_data;
}

function Wo_GetAppreciationValue()
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query_two    = mysqli_query($sqlConnect, " SELECT sum(`value`) AS appreciation_value FROM " . T_PRIVATE_THANKS . " where `recipient_id` = {$user_id} ");

    if (mysqli_num_rows($query_two) > 0) {
        $fetched_data = mysqli_fetch_assoc($query_two);
        return $fetched_data['appreciation_value'];
    }

    return false;
}

function Wo_CountPrivateData($type = 'referral')
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id = Wo_Secure($wo['user']['user_id']);
    $table = $type == 'referral' ? T_PRIVATE_REFERRALS : T_PRIVATE_THANKS;

    $query_one    = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . $table . " WHERE (`sender_id` = {$user_id} OR `recipient_id` = {$user_id})");
    $fetched_data = mysqli_fetch_assoc($query_one);
    return $fetched_data['count'];
}

function Wo_GetPrivateData($type = 'referral')
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id = Wo_Secure($wo['user']['user_id']);
    $table = $type == 'referral' ? T_PRIVATE_REFERRALS : T_PRIVATE_THANKS;
    $query_one    = mysqli_query($sqlConnect, "SELECT * FROM " . $table . " WHERE (`sender_id` = {$user_id} OR `recipient_id` = {$user_id})");
    while ($fetched_data = mysqli_fetch_assoc($query_one)) {
        $new_data = $fetched_data;
        $new_data['sender_data'] = Wo_userData($fetched_data['sender_id']);
        $new_data['receiver_data'] = Wo_userData($fetched_data['recipient_id']);
        $new_data['sender'] = $user_id == $fetched_data['sender_id'] ? TRUE : FALSE;
...
// leads functions - START
function Wo_IsCanPrivateLeadUpdate($Lead_id, $page)
{
    global $sqlConnect, $wo;
    $array = array('general', 'avatar', 'members', 'analytics', 'delete_Lead');
    if ($wo['loggedin'] == false) {
        return false;
    }
    if (empty($Lead_id) || !is_numeric($Lead_id) || $Lead_id < 0 || empty($page) || !in_array($page, $array)) {
        return false;
    }
    if (Wo_IsAdmin() || Wo_IsModerator()) {
        return true;
    ...
    $query = mysqli_query($sqlConnect, " SELECT COUNT(*) FROM " . T_PRIVATE_ADMINS . " WHERE `Lead_id` = {$Lead_id} AND `user_id` = {$user_id} AND `{$page}` = '1'");
    return (Wo_Sql_Result($query, '0') == 1) ? true : false;
}

function Wo_GetAllowedPrivateLeadPages($Lead_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    if (empty($Lead_id) || !is_numeric($Lead_id) || $Lead_id < 0) {
        return false;
    }
    $array = array('general' => 'general-setting', 'avatar' => 'avatar-setting', 'members' => 'Lead-members', 'analytics' => 'analytics', 'delete_Lead' => 'delete-Lead');
    $data = array();
    $user_id = $wo['user']['id'];
    $Lead_id = Wo_Secure($Lead_id);
    $query = mysqli_query($sqlConnect, " SELECT * FROM " . T_PRIVATE_ADMINS . " WHERE `Lead_id` = {$Lead_id} AND `user_id` = {$user_id}");
    $fetched_data = mysqli_fetch_assoc($query);
    if (!empty($fetched_data)) {
        foreach ($fetched_data as $key => $value) {
            if (in_array($key, array_keys($array)) && $value == 1) {
...
function Wo_RegisterPrivateLead($registration_data = array())
{
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    ...
    $fields                          = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data                            = '\'' . implode('\', \'', $registration_data) . '\'';
    $query                           = mysqli_query($sqlConnect, "INSERT INTO " . T_PRIVATE_LeadS . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    } else {
        return false;
...
function Wo_GetMyPrivateLeads()
{
    ...
    $data['lead']       = array();
    $user_id    = Wo_Secure($wo['user']['user_id']);
    $query_text = "SELECT `id` FROM " . T_LEADS . " WHERE `user_id` = {$user_id} ";
    $query_one  = mysqli_query($sqlConnect, $query_text);
    if (mysqli_num_rows($query_one) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            if (is_array($fetched_data)) {
                $data['type'] = 'self';
                $data['lead'][] = Wo_PrivateLeadData($fetched_data['id']);
            }
        }
    } else {
...
function Wo_IsPrivateLeadOnwer($Lead_id = 0, $user_id = 0)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query = mysqli_query($sqlConnect, " SELECT COUNT(`user_id`) FROM " . T_LEADS . " WHERE `id` = {$Lead_id} AND `user_id` = {$user_id}");
    return (Wo_Sql_Result($query, '0') == 1 || Wo_IsLeadUserExists($user_id, $Lead_id)) ? true : false;
}

function Wo_PrivateLeadExists($Lead_name = '')
{
    global $sqlConnect;
    if (empty($Lead_name)) {
        return false;
    }
    $Lead_name = Wo_Secure($Lead_name);
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_LeadS . " WHERE `Lead_name`= '{$Lead_name}'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}


function Wo_PrivateLeadIdFromLeadname($Lead_name = '')
{
    global $sqlConnect;
    if (empty($Lead_name)) {
        return false;
    ...
    $Lead_name = Wo_Secure($Lead_name);
    $query      = mysqli_query($sqlConnect, "SELECT `id` FROM " . T_PRIVATE_LeadS . " WHERE `Lead_name` = '{$Lead_name}'");
    return Wo_Sql_Result($query, 0, 'id');
}

function Wo_PrivateLeadData($Lead_id = 0)
{
    global $wo, $sqlConnect, $cache;
    ...
    $data            = array();
    $Lead_id        = Wo_Secure($Lead_id);
    $query_one       = "SELECT * FROM " . T_LEADS . " WHERE `id` = {$Lead_id}";
    $hashed_Lead_Id = md5($Lead_id);
    if ($wo['config']['cacheSystem'] == 1) {
    ...
    $fetched_data['avatar']      = Wo_GetMedia($fetched_data['avatar']);
    $fetched_data['cover']       = Wo_GetMedia($fetched_data['cover']);
    $fetched_data['url']         = Wo_SeoLink('index.php?link1=private-timeline&u=' . $fetched_data['Lead_name']);
...
function Wo_IsPrivateLeadJoined($Lead_id = 0, $user_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $Lead_id  = Wo_Secure($Lead_id);
    $query_one = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_MEMBERS . " WHERE `user_id` = '{$user_id}' AND `Lead_id` = {$Lead_id}");
    return (Wo_Sql_Result($query_one, 0) == 1) ? true : false;
}

function Wo_GetUserIdFromPrivateLeadId($Lead_id = 0)
{
    global $sqlConnect;
    if (empty($Lead_id) or !is_numeric($Lead_id) or $Lead_id < 1) {
        return false;
    }
    $Lead_id      = Wo_Secure($Lead_id);
    $query_one     = "SELECT `user_id` FROM " . T_PRIVATE_LeadS . " WHERE `id` = {$Lead_id}";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql_query_one) == 1) {
        $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
        return $sql_fetch_one['user_id'];
...
function Wo_DeletePrivateLead($Lead_id = 0)
{
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $Lead_id = Wo_Secure($Lead_id);
    if (Wo_IsAdmin() === false && Wo_IsModerator() === false) {
        if (Wo_IsPrivateLeadOnwer($Lead_id) === false) {
            return false;
        }
    }
    $query_one_delete_photos = mysqli_query($sqlConnect, " SELECT `avatar`,`cover` FROM " . T_PRIVATE_LeadS . " WHERE `id` = {$Lead_id}");
    $fetched_data            = mysqli_fetch_assoc($query_one_delete_photos);
    if (isset($fetched_data['avatar']) && !empty($fetched_data['avatar']) && $fetched_data['avatar'] != $wo['LeadDefaultAvatar']) {
        @unlink($fetched_data['avatar']);
    }
    if (isset($fetched_data['cover']) && !empty($fetched_data['cover']) && $fetched_data['cover'] != $wo['userDefaultCover']) {
        @unlink($fetched_data['cover']);
    }
    $query_two_delete_media = mysqli_query($sqlConnect, " SELECT `postFile` FROM " . T_POSTS . " WHERE `private_id` = {$Lead_id}");
    if (mysqli_num_rows($query_two_delete_media) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query_two_delete_media)) {
            if (isset($fetched_data['postFile']) && !empty($fetched_data['postFile'])) {
                @unlink($fetched_data['postFile']);
    ...
    $query_four_delete_media = mysqli_query($sqlConnect, "SELECT `id`,`post_id` FROM " . T_POSTS . " WHERE `private_id` = {$Lead_id}");
    if (mysqli_num_rows($query_four_delete_media) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query_four_delete_media)) {
            $delete_posts = Wo_DeletePost($fetched_data['id']);
            $delete_posts = Wo_DeletePost($fetched_data['post_id']);
    ...
    if ($wo['config']['cacheSystem'] == 1) {
        $cache->delete(md5($Lead_id) . '_PRIVATE_Lead_Data.tmp');
        $query_two = mysqli_query($sqlConnect, "SELECT `id`,`post_id` FROM " . T_POSTS . " WHERE `private_id` = {$Lead_id}");
        if (mysqli_num_rows($query_two) > 0) {
            while ($fetched_data_two = mysqli_fetch_assoc($query_two)) {
                $cache->delete(md5($fetched_data_two['id']) . '_PRIVATE_Lead_Data.tmp');
                $cache->delete(md5($fetched_data_two['post_id']) . '_PRIVATE_Lead_Data.tmp');
            }
        }
    }
    $query_one = mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_LeadS . " WHERE `id` = {$Lead_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_NOTIFICATION . " WHERE `private_id` = {$Lead_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_PRIVATE_ADMINS . " WHERE `Lead_id` = {$Lead_id}");
    $query_one .= mysqli_query($sqlConnect, "DELETE FROM " . T_POSTS . " WHERE `private_id` = {$Lead_id}");
    if ($query_one) {
        return true;
    }
}

function Wo_CountPrivateLeadMembers($Lead_id = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($Lead_id) or !is_numeric($Lead_id) or $Lead_id < 1) {
    ...
    $Lead_id     = Wo_Secure($Lead_id);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`Lead_id`) AS count FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id} AND `active` = '1' ");
    $fetched_data = mysqli_fetch_assoc($query);
    return $fetched_data['count'];
}

function Wo_PrivateLeadExistsByID($id)
{
    global $sqlConnect;
    if (empty($id)) {
        return false;
    ...
    $id    = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_LEADS . " WHERE `id`= '{$id}' AND `active` = '1'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}



function Wo_GetPrivateLeadMembers($Lead_id = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($Lead_id) or !is_numeric($Lead_id) or $Lead_id < 1) {
    ...
    $Lead_id  = Wo_Secure($Lead_id);
    $query     = " SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id} AND `active` = '1'";
    $sql_query = mysqli_query($sqlConnect, $query);
    while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
        $data[] = Wo_UserData($fetched_data['user_id']);
    }
...
function Wo_GetPrivateLeadMember($Lead_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $Lead_id  = Wo_Secure($Lead_id);
    $query     = " SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id} AND `user_id` = {$user_id} AND `active` = '1'";
    $sql_query = mysqli_query($sqlConnect, $query);
    return mysqli_fetch_assoc($sql_query);
}

function Wo_GetPrivateLeadMemberCategory($user_id)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
...
function Wo_CountPrivateLeadsNotMember($Lead_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $Lead_id     = Wo_Secure($Lead_id);
    $query_one    = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as count FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$user_id} AND `active` = '1' AND `following_id` NOT IN (SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id})");
    $fetched_data = mysqli_fetch_assoc($query_one);
    return $fetched_data['count'];
}
function Wo_GetPrivateLeadsNotMember($Lead_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $Lead_id  = Wo_Secure($Lead_id);
    $user_id   = Wo_Secure($wo['user']['user_id']);
    $query_one = mysqli_query($sqlConnect, "SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$user_id} AND `active` = '1' AND `following_id` NOT IN (SELECT `user_id` FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id})");
    while ($fetched_data = mysqli_fetch_assoc($query_one)) {
        $data[] = Wo_UserData($fetched_data['following_id']);
    }
    return $data;
...
function Wo_CanBeOnPrivateLead($Lead_id)
{
    global $sqlConnect;
    if (empty($Lead_id)) {
        return false;
    ...
    $Lead_id = Wo_Secure($Lead_id);
    if (Wo_IsPrivateLeadOnwer($Lead_id)) {
        return true;
    }
    $Lead = Wo_PrivateLeadData($Lead_id);
    if (empty($Lead)) {
        return false;
    }

    if (Wo_IsPrivateLeadJoined($Lead_id) === true) {
        return true;
    } else {
        return false;
    }
...
function Wo_GetPrivateLeadPostPublisherBox($Lead_id = 0)
{
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    if (!is_numeric($Lead_id) or $Lead_id < 1 or !is_numeric($Lead_id)) {
        return false;
    }
    $Lead_id = Wo_Secure($Lead_id);
    $continue = false;
    if (Wo_CanBeOnPrivateLead($Lead_id) === true) {
        $Lead = Wo_PrivateLeadData($Lead_id);
...
function Wo_CountPrivateLeadPosts($Lead_id = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($Lead_id) or !is_numeric($Lead_id) or $Lead_id < 1) {
    ...
    $Lead_id     = Wo_Secure($Lead_id);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`id`) AS count FROM " . T_POSTS . " WHERE `private_id` = {$Lead_id}");
    $fetched_data = mysqli_fetch_assoc($query);
    return $fetched_data['count'];
}

function Wo_CountPrivateLeadJoinedThisWeek($Lead_id = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    $time = strtotime("-1 week");
    ...
    $Lead_id     = Wo_Secure($Lead_id);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`Lead_id`) AS count FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id} AND `active` = '1' AND (`time` between {$time} AND " . time() . ")");
    $fetched_data = mysqli_fetch_assoc($query);
    return $fetched_data['count'];
}

function Wo_UpdatePrivateLeadData($Lead_id = 0, $update_data)
{
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (!empty($update_data['category'])) {
        if (!array_key_exists($update_data['category'], $wo['Lead_categories'])) {
            $update_data['category'] = 1;
        }
    }
    $update = array();
    ...
    $impload   = implode(', ', $update);
    $query_one = " UPDATE " . T_PRIVATE_LeadS . " SET {$impload} WHERE `id` = {$Lead_id} ";
    $query     = mysqli_query($sqlConnect, $query_one);
    if ($wo['config']['cacheSystem'] == 1) {
        $cache->delete(md5($Lead_id) . '_PRIVATE_Lead_Data.tmp');
    }
...
function Wo_PrivateLeadActive($Lead_name)
{
    global $sqlConnect;
    if (empty($Lead_name)) {
        return false;
    ...
    $Lead_name = Wo_Secure($Lead_name);
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_LeadS . "  WHERE `Lead_name` = '{$Lead_name}' AND `active` = '1'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_GetPrivateLeadAdminInfo($user_id, $Lead_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || empty($Lead_id) || !is_numeric($Lead_id) || empty($user_id) || !is_numeric($user_id)) {
        return false;
    }
    $Lead_id = Wo_Secure($Lead_id);
    $user_id = Wo_Secure($user_id);
    $sql     = " SELECT * FROM " . T_PRIVATE_ADMINS . " WHERE `Lead_id` = {$Lead_id} AND `user_id` = {$user_id}";
    $data    = array();
    $query   = mysqli_query($sqlConnect, $sql);
    return mysqli_fetch_assoc($query);
}

function Wo_IsPrivateLeadNameExist($username, $active = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($username)) {
    ...
    $query   = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as userLeads FROM " . T_PRIVATE_LeadS . " WHERE `Lead_name` = '{$username}' {$active_text}") or die(mysqli_error($sqlConnect));
    $fetched_data = mysqli_fetch_assoc($query);
    if ($fetched_data['userLeads'] > 0) {
        return array(
            true,
            'type' => 'private_Lead'
        );
...
function Wo_GetPrivateLeadSettingMembers($Lead_id = 0, $limit = 0, $offset = 0)
{
    global $wo, $sqlConnect;
    $data = array();
    if (empty($Lead_id) or !is_numeric($Lead_id) or $Lead_id < 1) {
    ...
    $Lead_id  = Wo_Secure($Lead_id);
    $query     = " SELECT `user_id`,`id` FROM " . T_PRIVATE_MEMBERS . " WHERE `Lead_id` = {$Lead_id} AND `active` = '1' $offset_query $limit_query";
    $sql_query = mysqli_query($sqlConnect, $query);
    while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
        $user_data = Wo_UserData($fetched_data['user_id']);
        $user_data['member_id'] = $fetched_data['id'];
...
function Wo_UpdatePrivateLeadAdminData($Lead_id, $update_data, $user_id)
{
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $impload   = implode(', ', $update);
    $query_one = " UPDATE " . T_PRIVATE_ADMINS . " SET {$impload} WHERE `Lead_id` = {$Lead_id} AND `user_id` = '{$user_id}' ";
    $query     = mysqli_query($sqlConnect, $query_one);
    if ($query) {
        return true;
    } else {
...
function Wo_CanSendInviteLead($user_id = 0, $Lead_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (Wo_IsPrivateLeadJoined($Lead_id, $logged_user_id) === false) {
        return false;
    }

    if (Wo_IsPrivateLeadJoined($Lead_id, $user_id) === true) {
        return false;
    }
...
function Wo_RegisterPrivateLeadInvite($user_id = 0, $Lead_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id  = Wo_Secure($user_id);
    $Lead_id = Wo_Secure($Lead_id);
    if (Wo_CanSendInvite($user_id, $Lead_id) == false) {
        return false;
    }
...
function Wo_RegisterInviteLead($registration_data = array())
{
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
...
function Wo_GenerateRandomStringLead($length = 10)
{
    $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $charactersLength = strlen($characters);
    $randomString = '';
...
function Wo_isUserInvitedLead($user_id = 0, $Lead_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id  = Wo_Secure($user_id);
    $Lead_id = Wo_Secure($Lead_id);
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_INVITES . " WHERE `recipient_id` = '{$user_id}' && `Lead_id` = '{$Lead_id} ORDER BY `time` DESC LIMIT 1'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_PrivateLeadInvitationExists($token = '', $Lead_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_PRIVATE_INVITES . " WHERE `token` = '{$token}' && `recipient_id` = '{$user_id}' && `Lead_id` = '{$Lead_id}' && `active` = '0'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}

function Wo_GetPrivateLeadInvitation($token = '', $Lead_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query      = mysqli_query($sqlConnect, "SELECT * FROM " . T_PRIVATE_INVITES . " WHERE `token` = '{$token}' && `recipient_id` = '{$user_id}' && `Lead_id` = '{$Lead_id}' && `active` = '0'");
    return mysqli_fetch_assoc($query);
}

function Wo_GetAllowedCategoriesLead($Lead_id = 0)
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $Lead_id = Wo_Secure($Lead_id);
    $categories = mysqli_query($sqlConnect, "SELECT B.`id`, B.`lang_key` FROM " . T_PRIVATE_ALLOWED_CATEGORY . " as A JOIN " . T_PRIVATE_JOB_CATEGORY . " AS B ON A.`jobcategory_id` = B.`id` WHERE A.`Lead_id` = '{$Lead_id}' AND A.active = '0'");
    if ($categories) {
        while ($fetched_data = mysqli_fetch_assoc($categories)) {
            $data[$fetched_data['id']] = $wo['lang'][$fetched_data['lang_key']];
        }
...
function Wo_LeadExists($group_name = '')
{
    global $sqlConnect;
    if (empty($group_name)) {
        return false;
    ...
    $group_name = Wo_Secure($group_name);
    $query      = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_LEADS . " WHERE `id`= '{$group_name}'");
    return (Wo_Sql_Result($query, 0) == 1) ? true : false;
}
// END
...
function Wo_Getcreated_lead_data($lead_id){
    ...
    $query_text = "SELECT *,A.`id` as leadId,
                            ...
                            A.`product_category`,
                            B.`id` as job_id,
                            B.`lang_key` as job_cat,
                            C.`user_id` as user_id,
                            D.`id` as group_id,
                            D.`lang_key` as group_cat,
                            E.`id` as product_id,
                            E.`lang_key` as product_cat
                    FROM ". T_LEADS ." as A JOIN ". T_JOB_CATEGORY ." as B
                     ON A.`job_category` = B.`id` 
                     JOIN ".T_USERS." as C
                     ON A.user_id=C.`user_id`
                     JOIN ".T_GROUPS_CATEGORY." as D
                     ON A.`group_category`=D.`id`
    ...
    if (mysqli_num_rows($query_one) > 0) {
        $fetched_data = mysqli_fetch_assoc($query_one);
        if (is_array($fetched_data)) {
            $data['product_cat'] = $fetched_data['product_cat'];
            $data['Group_cat'] = $fetched_data['group_cat'];
            $data['category_name'] = $fetched_data['job_cat'];
            $data['user_name'] = $fetched_data['username'];
            $data['lead']= Wo_PrivateLeadData($fetched_data['leadId']);
            //get product lang
            $query2_text= "SELECT `lang_key`,`english` FROM ".T_LANGS."
            where lang_key ='{$data['product_cat']}'";
            ...
            where lang_key ='{$data['category_name']}'";
