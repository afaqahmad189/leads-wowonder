            ...
            if (empty($get_session_data) || !empty($get_session_data)) {
                ...
                if (!empty($to_data['android_n_device_id']) || !empty($to_data['ios_n_device_id']) || !empty($to_data['web_device_id'])) {
                    ...
                    if (!empty($sql_get_notification_for_push['type'])) {
                        ...
                        // private group - START
                        if ($sql_get_notification_for_push['type'] == 'private_group_invited') {
                            $group                                      = Wo_PrivateGroupData($sql_get_notification_for_push['private_id']);
                            $sql_get_notification_for_push['type_text'] = str_replace('{group_name}', $group['name'], $wo['lang']['private_group_invited']);
                        }
                        
                        if ($sql_get_notification_for_push['type'] == 'send_referral_notice') {
                            $sql_get_notification_for_push['type_text'] = $wo['lang']['send_referral_notice'];
                        }
                        // END
...
function Wo_IsReportExists($id = false, $type = 'user')
{
    ...
    if ($type == 'user') {
        ...
        // get reports for private groups
        $sql       = " SELECT `id` FROM " . T_REPORTS . " WHERE `private_id` = {$id} AND `user_id` = {$user}";
        $data_rows = mysqli_query($sqlConnect, $sql);
        $match     = mysqli_num_rows($data_rows) > 0;
    }
    return $match;
...
function Wo_IsPrivateGroupUserExists($user_id = false, $group_id = false)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($user_id) || !is_numeric($group_id)) {
        return false;
    }
    $sql       = " SELECT `id` FROM " . T_PRIVATE_ADMINS . " WHERE `user_id` = {$user_id} AND `group_id` = {$group_id} ";
    $data_rows = mysqli_query($sqlConnect, $sql);
    return mysqli_num_rows($data_rows) > 0;
}

function Wo_CheckPrivateGroupAdminPassword($password = false, $group_id = false)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($password) || !is_numeric($group_id)) {
        return false;
    ...
    $group_id = Wo_Secure($group_id);
    $match    = false;
    if (Wo_IsPrivateGroupUserExists($user_id, $group_id)) {
        $sql  = "SELECT `password` FROM " . T_USERS . " WHERE `user_id` = {$user_id}";
        $data = mysqli_query($sqlConnect, $sql);
        if (mysqli_num_rows($data) == 1) {
            $fetched_data = mysqli_fetch_assoc($data);
