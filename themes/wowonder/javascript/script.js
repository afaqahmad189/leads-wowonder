...
function Wo_GetMorePosts() {
    ...
    } else if (typeof($('#posts').attr('data-story-private')) == "string") {
        // for private groups 
        private_id = $('#posts').attr('data-story-private');
...
function Wo_FilterPostBy(filter_by) {
    ...
    if (typeof($('#posts').attr('data-story-user')) == "string") {
        ...
        // for private groups 
        id = $('#posts').attr('data-story-private');
        type = 'private_group';
    } else if (typeof($('#posts').attr('data-story-event')) == "string") {
...
function Wo_SearchForPosts(query) {
    ...
    if ($('.page-margin').attr('data-page') == "timeline") {
        ...
        type = 'group'
    } else if ($('.page-margin').attr('data-page') == "private_group") {
        type = 'private_group'
    } else {
        return false;
    }
...
// remove private group member
function Wo_DeletePrivateJoinedUser(user_id, group_id) {
    $.get(Wo_Ajax_Requests_File(), { f: 'private_group', s: 'delete_joined_user', user_id: user_id, group_id: group_id }, function(data) {
        if (data.status == 200) {
            $('#member-' + user_id).fadeOut(300, function() {
                $(this).remove();
            });
...
function Wo_StartBar() { $(".loader").css("display", "block") }

function Wo_FinishBar() { $(".loader").css("display", "none") }
