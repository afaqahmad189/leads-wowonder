...
if ($f == 'posts') {
    ...
    if ($s == 'insert_new_post') {
        ...
        $page_id       = 0;
        $group_id      = 0;
        $private_id    = 0; // init private group id 
        $event_id      = 0;
        $invalid_file  = false;
        $errors        = false;
        $image_array   = array();
        ...
        } else if (isset($_POST['private_id']) && !empty($_POST['private_id'])) {
            // for private posts
            $private_id = Wo_Secure($_POST['private_id']);
            $_POST['postPrivacy'] = 2;
        }
        ...
        if (empty($errors) && $invalid_file == false) {
            ...
            $post_data = array(
                ...
                'private_id' => Wo_Secure($private_id), // insert private group id
            ...
            if ($wo['config']['notify_new_post'] == 1) {
                if (empty($wo['story']['page_id']) && empty($wo['story']['group_id']) && empty($wo['story']['private_id']) && empty($wo['story']['event_id']) && $wo['story']['postPrivacy'] < 3) {
    ...
    if ($s == 'load_more_posts') {
        ...
        if (!empty($_GET['filter_by_more']) && $_GET['filter_by_more'] == 'story' && isset($_GET['story_id']) && isset($_GET['user_id'])) {
            ...
            // for private groups
            if (!empty($_GET['private_id']) && $_GET['private_id'] > 0) {
                $private_id = Wo_Secure($_GET['private_id']);
            }
            ...
            $postsData = array(
                ...
                'group_id' => $group_id,
                'private_id' => $private_id, // send private group id
                'page_id' => $page_id,
                'event_id' => $event_id,
                'after_post_id' => Wo_Secure($_GET['after_post_id']),
                'ad-id' => $ad_id,
        ...
        if (!empty($_GET['post_id']) && Wo_CheckMainSession($hash_id) === true) {
            ...
            if (!empty($_GET['type'])) {
                $types_array = array(
                    ...
                    'page',
                    'group',
                    'private_group', // for private groups
                    'event'
                );
                if (in_array($_GET['type'], $types_array)) {
                    $type = $_GET['type'];
    ...
    if ($s == 'filter_posts') {
        if (!empty($_GET['filter_by']) && isset($_GET['id'])) {
            ...
            if (!empty($_GET['type'])) {
                ...
                } else if ($_GET['type'] == 'private_group') {
                    $options['private_id'] = $_GET['id'];
                }
            }
