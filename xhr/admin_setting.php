...
if ($f == 'admin_setting' and (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'approve_receipt') {
        if (!empty($_GET['receipt_id'])) {
            ...
            if ($receipt) {
                // for private groups - start
                if ($receipt->type == 'group') {
                    if ($receipt->mode == 'create') {
                        $group = $db->where('payment_id', $receipt->id)->getOne(T_PRIVATE_GROUPS);
                    } else {
                        $group = $db->where('id', $receipt->group_id)->getOne(T_PRIVATE_GROUPS);
                    }
                    ...
                    if (!empty($group)) {
                        ...
                        if ($receipt->mode == 'create') {
                            $db->where('id', $group->id)->update(T_PRIVATE_GROUPS, array('active' => 1));
                            ...
                            $notification_data_array = array(
                                ...
                                'text' => $wo['lang']['bank_pro_create'],
                                'type2' => 'no_name'
                            );
                            Wo_RegisterNotification($notification_data_array);
                        } else {
                            $db->where('group_id', $group->id)->where('user_id', $receipt->user_id)->update(T_PRIVATE_MEMBERS, array('active' => 1));
                        ...
                        $data = array(
                            'status' => 400,
                            'error' => $error_icon . $wo['lang']['approval_error']
                        );
                    }
                } else {
                    $updated = $db->where('id', $id)->update('bank_receipts', array('approved' => 1, 'approved_at' => time()));
    ...
    if ($s == 'add_new_category') {
        ...
        // add private group table in array
        $types = array('page' => T_PAGES_CATEGORY, 'group' => T_GROUPS_CATEGORY, 'private-group' => T_PRIVATE_CATEGORY, 'blog' => T_BLOGS_CATEGORY, 'product' => T_PRODUCTS_CATEGORY, 'job' => T_JOB_CATEGORY);
        ...
        if (!empty($_GET['type']) && in_array($_GET['type'], array_keys($types))) {
            if ($_POST['lang_key'] != 'other' && $_POST['lang_key'] != 'all_') {
                ...
                if (!empty($category)) {
                    ...
                    // reset category for private groups
                    if ($_GET['type'] == 'private-group') {
                        $db->where('category', $category->id)->update(T_PRIVATE_GROUPS, array('category' => 1));
                    }
                    if ($_GET['type'] == 'blog') {
    ...
    if ($s == 'create_subadmin' && isset($_GET['user_id']) && Wo_CheckMainSession($hash_id) === true) {
        if (Wo_CreateSubadmin($_GET['user_id']) === true) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
...
//            $data['html'] = $html;
    ...
    // delete private group - START
    if ($s == 'delete_private_group' && isset($_GET['group_id'])) {
        if (Wo_DeletePrivateGroup($_GET['group_id']) === true) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    // add new private group job category - START
    if ($s == 'add_new_job_category') {
        $data['status'] = 400;
        $data['message'] = 'Please check your details';
        $types = array('private-group');
        $all_categories = array('private-group' => $wo['all_private_categories']);
        if (!empty($_GET['type']) && in_array($_GET['type'], $types) && in_array($_GET['type'], array_keys($all_categories)) && !empty($_POST['category_id']) && in_array($_POST['category_id'], array_keys($all_categories[$_GET['type']]))) {
            $type = Wo_Secure($_GET['type']);
            $add = false;
            $insert_data = array();
            ...
            if ($add == true && !empty($insert_data)) {
                $id = $db->insert(T_LANGS, $insert_data);
                $db->insert(T_PRIVATE_JOB_CATEGORY, array(
                    'lang_key' => $id,
                    'category_id' => Wo_Secure($_POST['category_id'])
                ));
                $db->where('id', $id)->update(T_LANGS, array('lang_key' => $id));
    ...
    if ($s == 'delete_job_category' && !empty($_POST['lang_key'])) {
        $types = array('private-group');
        if (!empty($_GET['type']) && in_array($_GET['type'], $types)) {
            $lang_key = Wo_Secure($_POST['lang_key']);
            $category = $db->where('lang_key', $lang_key)->getOne(T_PRIVATE_JOB_CATEGORY);

            if (!empty($category)) {
                $db->where('lang_key', $lang_key)->delete(T_LANGS);
                $db->where('id', $category->id)->delete(T_PRIVATE_JOB_CATEGORY);

                // reset private category
                $db->where('category', $category->id)->delete(T_PRIVATE_MEMBERS);
                $db->where('jobcategory_id', $category->id)->delete(T_PRIVATE_ALLOWED_CATEGORY);
                $data['status'] = 200;
            }
        }
        header("Content-type: application/json");
    ...
    if ($s == 'delete_suggestion' && !empty($_POST['id'])) {
        $id = Wo_Secure($_POST['id']);
        $suggestion = $db->where('id', $id)->getOne(T_PRIVATE_SUGGESTIONS);

        if (!empty($suggestion)) {
            $db->where('id', $id)->delete(T_PRIVATE_SUGGESTIONS);
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'add_new_product_category') {
        if (Wo_CheckSession($hash_id) === true) {
            $mysqli = Wo_ProductCategoryNameFromDB();
            if (in_array($_POST['lang'], $mysqli)) {
                $data['status']  = 400;
                $data['message'] = 'This product category is already created.';
            } else {
                $cat_name = Wo_Secure($_POST['category']);
//                $lang_name = strtolower($lang_name);
                $query     = $db->insert(T_PRODUCT_CATEGORY, array('name' => $cat_name));
//                $query     = mysqli_query($sqlConnect, "ALTER TABLE " . T_LANGS . " ADD `$lang_name` TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL;");
                if ($query) {
                    $data['status'] = 200;
                }
    ...
    if ($s == 'update_product_category') {
        if (Wo_CheckSession($hash_id) === true) {
                $cat_name = Wo_Secure($_POST['category']);
            $id = Wo_Secure($_POST['id']);
//                $lang_name = strtolower($lang_name);
            $updated = $db->where('id', $id)->update('Wo_Product_Category', array('name' => $cat_name));
//                $query     = mysqli_query($sqlConnect, "ALTER TABLE " . T_LANGS . " ADD `$lang_name` TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL;");
                if (true) {
                    $data['status'] = 200;
                }
    ...
    if ($s == 'delete_product_category') {
        if (Wo_CheckMainSession($hash_id) === true) {
                $id = Wo_Secure($_GET['id']);
                $db->where('id', $id)->delete(T_PRODUCT_CATEGORY);
                $query     = 1;
                if ($query) {
                    $data['status'] = 200;
                }
    ...
    if ($s == 'approve_lead') {
        if (!empty($_GET['receipt_id'])) {
            $id = Wo_Secure($_GET['receipt_id']);
            $receipt = $db->where('id', $id)->getOne('wo_leads', array('*'));
            ...
            if ($receipt) {
                    $updated = $db->where('id', $id)->update('wo_leads', array('status' => 1, 'approved_at' => time()));
                    if ($updated === true) {
                            $notification_data_array = array(
                                'recipient_id' => $receipt->user_id,
                                'type' => 'admin_notification',
                                'url' => 'index.php?link1=leads',
                                'text' => $wo['lang']['lead_approve_success'],
                                'type2' => 'no_name'
                            );
                            Wo_RegisterNotification($notification_data_array);
                    }
    ...
    if ($s == 'delete_lead') {
        if (!empty($_GET['receipt_id'])) {
            ...
            $notification_data_array = array(
                ...
                'type' => 'admin_notification',
                'url' => 'index.php',
                'text' => $wo['lang']['lead_decline'],
                'type2' => 'no_name'
            );
            Wo_RegisterNotification($notification_data_array);
            $db->where('id', $id)->delete('wo_leads');
            $data = array(
                'status' => 200
            );
        }
