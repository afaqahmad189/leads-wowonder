...
if (isset($_GET['u'])) {
    ...
    $check_private_group = Wo_IsNameExist($_GET['u'], 0);
    ...
    if (in_array(true, $check_user)) {
        ...
        } else if ($check_user['type'] == 'private_group') {
    ...
    } else if (in_array(true, $check_private_group)) {
        if ($check_private_group['type'] == 'private_group') {
...
if (!empty($_GET['type']) && in_array($_GET['type'], array('activities', 'mutual_friends', 'following', 'followers', 'videos', 'photos', 'likes', 'groups', 'family_list', 'requests', 'private_group'))) {
